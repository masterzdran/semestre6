...
as
	begin
		...
		declare @ing_name char(30), @sup_name char(30), @average_price smallmoney
		...
		while(@@FETCH_STATUS=0)
			begin
			select @average_price = AVG(LOT.PRICE) from LOT
				where LOT.ID=@ing_id
			...
			if @price > @_price
				...
				select @sup_name = SUPPLIER.NAME from SUPPLIER
