...
sqlcmd.exe  -S %server% -E -d %db% -i %CD%\triggers.sql >>output.txt
ECHO Done! >>output.txt
...
REM sqlcmd.exe  -S %server% -E -d %db% -i %CD%\Ex4-d.sql >>output.txt
REM sqlcmd.exe  -S %server% -E -d %db% -i %CD%\Ex4-e.sql >>output.txt
