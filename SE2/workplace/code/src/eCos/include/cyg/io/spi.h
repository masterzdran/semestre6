...
//	bitbang
typedef struct {
	void (*mosi)(cyg_uint32 data, cyg_uint32 mask);
	void (*sclk)(int value);
	int  (*miso)(void);
} spi_bitbang_funcs ;
...
typedef struct {
	cyg_spi_bus     	spi_bus;
	spi_bitbang_funcs * spi_dev;
} cyg_spi_bitbang_bus_t;
...
typedef struct {
	...
	void		(*spi_cs)(int);
} cyg_spi_bitbang_dev_t;

externC void 
spi_bitbang_init_bus(cyg_spi_bitbang_bus_t *bus);
